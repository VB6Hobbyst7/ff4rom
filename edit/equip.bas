...
' If you put a two-handed weapon on, it will remove whatever is currently in
'  the actor's off hand. If you put an off-hand item on an actor who already
'  has a two-handed weapon on, it will remove the two-handed weapon.
...
sub FF4Rom.Equip(actor_index as UByte, item_index as UByte, arrow_ammo as UByte = 50, force_hand as String = "")
 ...
 if CanEquip(actor_index, item_index) then
  if weapons_range.InRange(item_index) and not bow_range.InRange(item_index) then
   'It's a weapon; figure out which hand to use (bows are treated like shields)
   if ambi then
    if lcase(left(force_hand, 1)) = "r" then
     use_right = true
    elseif lcase(left(force_hand, 1)) = "l" then
     use_right = false
    else
     if actors(actor_index).starting_gear(right_hand_slot) > 0 and actors(actor_index).starting_gear(left_hand_slot) = 0 then
      use_right = false
     else 'if actors(actor_index).starting_gear(right_hand_slot) = 0 and actors(actor_index).starting_gear(left_hand_slot) > 0 then
      use_right = true
     end if
    end if
   else
   ...
   if use_right then
    ...
    if two_handed_range.InRange(item_index) then
     actors(actor_index).starting_gear(left_hand_slot) = 0
    ...
    if two_handed_range.InRange(item_index) then
     actors(actor_index).starting_gear(right_hand_slot) = 0
     actors(actor_index).right_ammo = 0
    end if
   end if
  elseif armors_range.InRange(item_index) or bow_range.InRange(item_index) then
   ...
   elseif arms_range.InRange(item_index) then
    ...
    'It must be a shield (or bow)
    if shield_range.InRange(item_index) then
     if not ambi then
      'Ambidextrous people can't use shields
      if characters(character_index).right_handed then
       actors(actor_index).starting_gear(left_hand_slot) = item_index
       actors(actor_index).left_ammo = 1
       if two_handed_range.InRange(actors(actor_index).starting_gear(right_hand_slot)) then
        actors(actor_index).starting_gear(right_hand_slot) = 0
        actors(actor_index).right_ammo = 0
       end if
      else
       ...
       if two_handed_range.InRange(actors(actor_index).starting_gear(left_hand_slot)) then
        actors(actor_index).starting_gear(left_hand_slot) = 0
        actors(actor_index).left_ammo = 0
       end if
      end if
