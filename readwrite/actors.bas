sub FF4Rom.ReadActors()
 ...
 for i as Integer = 1 to total_actors
  ...
  for j as Integer = 1 to 5
   actors(i).menu_command(j) = ByteAt(&h9FF50 + i * 5 + j - 1)
  next
 ...
 for i as Integer = 1 to total_names
  names(i) = ""
  for j as Integer = 0 to 5
   names(i) += chr(ByteAt(&h7A910 + (i - 1) * 6 + j))
  next
 next
...
sub FF4Rom.WriteActors()
 ...
 for i as Integer = 1 to total_actors
  ...
  WriteByte(&h8657 + i - 1, actors(i).name_index)
  WriteByte(&hB7D3F + i - 1, actors(i).name_index)
  
  temp = actors(i).load_slot
  if not actors(i).load_initial then temp += &h80
  WriteByte(&h0689A + i - 1, temp)
  ...
  WriteByte(&h0691D + i - 1, temp)
  
  for j as Integer = 1 to 5
   WriteByte(&h9FF50 + i * 5 + j - 1, actors(i).menu_command(j))
  next
 ...
 for i as Integer = 1 to total_names
  for j as Integer = 0 to 5
   ...
   WriteByte(&h7A910 + (i - 1) * 6 + j, asc(mid(names(i), j + 1, 1)))
  next
 next
